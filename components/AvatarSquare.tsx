...
/** Allow wildcard subdomains like v0.airtableusercontent.com, v6.airtableusercontent.com, etc. */
...
function isAllowedSrc(src?: string | null): string | undefined {
  ...
  try {
    const u = new URL(src);
    const { hostname } = u;
    if (ALLOWED_HOSTS.has(hostname) || ALLOWED_SUFFIXES.some((sfx) => hostname.endsWith(sfx))) {
      return src;
    }
    return undefined;
  } catch {
...
export default function AvatarSquare({
  ...
  const fontPx = useMemo(() => {
    ...
    if (variant === 'party') {
      if (len <= 2) return Math.round(size * 0.4);
      if (len === 3) return Math.round(size * 0.34);
      if (len === 4) return Math.round(size * 0.3);
...
function FallbackIcon({
  ...
  return variant === 'party' ? (
    <svg width={wh} height={wh} viewBox="0 0 24 24" aria-hidden="true">
