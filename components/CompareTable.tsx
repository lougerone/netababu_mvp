...
function renderTwitter(tw?: string | null) {
  ...
  if (/^https?:\/\//i.test(handle)) {
    try {
      handle = new URL(handle).pathname.replace(/^\/+/, '');
    } catch {}
  }
  ...
  return (
    <div ref={ref}>
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute z-[200] mt-1 w-full max-h-72 overflow-auto rounded-xl border border-black/10 bg-white shadow-card">
            {filtered.length === 0 ? (
              ...
              filtered.map((p, i) => (
                <button
                  ...
                  onClick={() => choose(p)}
                  className={`w-full px-3 py-2 text-left text-sm hover:bg-cream-200/60 ${
                    i === idx ? 'bg-cream-200/80' : ''
                  }`}
                >
                  <div className="font-medium">{p.name}</div>
                  <div className="text-[12px] text-ink-600/80">
...
function MultiSelect({ label, options, value, onToggle }: MultiProps) {
  ...
  return (
    <div ref={ref}>
      ...
      <button
        type="button"
        className="input-pill w-full truncate text-left"
        onClick={() => setOpen((o) => !o)}
        ...
        <div className="relative">
          <div className="absolute z-[200] mt-1 w-full max-h-72 overflow-auto rounded-xl border border-black/10 bg-white shadow-card">
            {options.map((o) => (
              <label
                key={String(o.key)}
                className="flex cursor-pointer items-center gap-2 px-3 py-2 text-sm hover:bg-cream-200/60"
              >
              ...
              shouldShow(f) ? (
                <tr key={`locked-${f}`} className="odd:bg-cream-100/50">
                  <th scope="row" className="px-4 py-3 font-medium">
                    {f === 'party' ? 'Party' : 'Current Position'}
                  </th>
                  <td className="px-4 py-3">{renderValue(f, A)}</td>
                  <td className="px-4 py-3">{renderValue(f, B)}</td>
                </tr>
              ) : null
            )}
            ...
            {activeOptional.map((c) =>
              shouldShow(c.key as string) ? (
                <tr key={`opt-${String(c.key)}`} className="odd:bg-cream-100/50">
                  <th scope="row" className="px-4 py-3">{c.label}</th>
                  <td className="px-4 py-3">{renderValue(String(c.key), A)}</td>
                  <td className="px-4 py-3">{renderValue(String(c.key), B)}</td>
                </tr>
              ) : null
            )}
          </tbody>
