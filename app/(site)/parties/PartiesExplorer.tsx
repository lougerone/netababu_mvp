...
<div className="px-5 sm:px-8 py-6 bg-cream-200/50 border-b border-ink-200">
  <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-4 w-full">
    {/* left cluster: search + selects (flex-1 so it grows) */}
    <div className="flex-1 min-w-[280px] flex flex-wrap items-center gap-3 sm:gap-4">
      <input
        value={q}
        onChange={(e) => setQ(e.target.value)}
        placeholder="Search by name, abbreviation, leader, state…"
        className="flex-1 min-w-[240px] rounded-full border border-ink-200 bg-white px-4 py-2.5 text-[15px] outline-none
      ...
      <select
        ...
        className="rounded-full border border-ink-200 bg-white px-3 py-2 text-sm"
      >
    ...
    {/* right cluster: actions (sticks to right using ml-auto) */}
