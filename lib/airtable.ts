...
function toText(v: any): string | undefined {
  ...
  if (typeof v === 'object' && 'name' in (v as any)) return toText((v as any).name);
  return undefined;
}
...
export async function listPoliticians(
  ...
  if (qSafe) {
    ...
    try {
      remoteRecords = await atFetchAll(T_POL, params, max === Infinity ? Infinity : Math.max(max, 100));
    } catch {
      ...
      try {
        ...
        // No remote filter; fetch and filter locally
        remoteRecords = await atFetchAll(T_POL, { pageSize: '100' }, max === Infinity ? Infinity : Math.max(max, 100));
      }
    }
  } else {
    remoteRecords = await atFetchAll(T_POL, { pageSize: '100' }, max === Infinity ? Infinity : Math.max(max, 100));
  ...
  if (qSafe && mapped.length < 12) {
    const allRecords = await atFetchAll(T_POL, { pageSize: '100' }, 1000);
...
export async function listParties(
  ...
  if (qSafe) {
    ...
    try {
      remoteRecords = await atFetchAll(T_PAR, params, max === Infinity ? Infinity : Math.max(max, 100));
    } catch {
      // Safe fallback
      params = {
        pageSize: '100',
      ...
      try {
        ...
        remoteRecords = await atFetchAll(T_PAR, { pageSize: '100' }, max === Infinity ? Infinity : Math.max(max, 100));
      }
    }
  } else {
    remoteRecords = await atFetchAll(T_PAR, { pageSize: '100' }, max === Infinity ? Infinity : Math.max(max, 100));
  ...
  if (qSafe && mapped.length < 12) {
    const allRecords = await atFetchAll(T_PAR, { pageSize: '100' }, 1000);
    const union = unionById(remoteRecords, allRecords);
